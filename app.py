...
from models import Contact, todo, UserMixin
...
if __name__ == '__main__':
    ...
    def register():
        ...
        if form.validate_on_submit():
            new_user = UserMixin(email_address=form.email_address.data, name=form.name.data,
    ...
    def login():
        form = LoginForm()
        if form.validate_on_submit():
            user = UserMixin.query.filter_by(email_address=form.email_address.data).first()
    ...
    def reset_password():
        ...
        if form.validate_on_submit():
            user = UserMixin.query.filter_by(email_address=current_user.email_address).first()
